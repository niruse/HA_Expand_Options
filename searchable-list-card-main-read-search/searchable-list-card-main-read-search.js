...
class SearchableListCardRead extends LitElement {
...
/*
    async _changeItemStatus(ev) {
        ...
        this._keepLastSearch(this.searchText)ֿ
        await this._getListItems()
    }
*/
    async _changeItemStatus(ev) {
        ...
        const newStatus = ev.currentTarget["__checked"] ? 'completed' : 'needs_action';
    
        // Call the service to update the status of the item
        await this.hass.callService("todo", "update_item", {
            entity_id: this.config.entity,
            item: ev.target.id,
            status: newStatus
        ...
        // Ensure last search is retained and re-fetch the list items
...
customElements.define("searchable-list-card-read", SearchableListCardRead);

// Updated card configuration
window.customCards = window.customCards || [];
window.customCards.push({
    type: "searchable-list-card-read",
    name: "Searchable List Card Read",
    preview: true,
